import Foundation
import MLX
import MLXNN
import MLXRandom

nonisolated class BARTModel: Module {
    let config: BARTConfig
    let sharedEmbedding: Embedding
    let encoderPositionalEmbedding: Embedding
    let decoderPositionalEmbedding: Embedding
    let encoderLayers: [BARTEncoderLayer]
    let decoderLayers: [BARTDecoderLayer]
    let encoderNorm: LayerNorm
    let decoderNorm: LayerNorm
    let lmHead: Linear
    
    init(config: BARTConfig) {
        self.config = config
        
        // Shared embedding for encoder and decoder
        self.sharedEmbedding = Embedding(embeddingCount: config.vocabSize, dimensions: config.dModel)
        
        // Positional embeddings
        self.encoderPositionalEmbedding = Embedding(embeddingCount: config.maxPositionEmbeddings, dimensions: config.dModel)
        self.decoderPositionalEmbedding = Embedding(embeddingCount: config.maxPositionEmbeddings, dimensions: config.dModel)
        
        // Encoder layers
        self.encoderLayers = (0..<config.encoderLayers).map { _ in
            BARTEncoderLayer(dModel: config.dModel, numHeads: config.encoderAttentionHeads, dFF: config.encoderFFNDim)
        }
        
        // Decoder layers
        self.decoderLayers = (0..<config.decoderLayers).map { _ in
            BARTDecoderLayer(dModel: config.dModel, numHeads: config.decoderAttentionHeads, dFF: config.decoderFFNDim)
        }
        
        // Layer norms
        self.encoderNorm = LayerNorm(dimensions: config.dModel)
        self.decoderNorm = LayerNorm(dimensions: config.dModel)
        
        // Language model head
        self.lmHead = Linear(config.dModel, config.vocabSize, bias: false)
        
        super.init()
    }
    
    func encode(_ inputIds: MLXArray, mask: MLXArray? = nil) -> MLXArray {
        let seqLen = inputIds.shape[1]
        let positions = MLXArray(0..<seqLen).reshaped([1, seqLen])
        
        // Embeddings
        var hidden = sharedEmbedding(inputIds)
        hidden = hidden + encoderPositionalEmbedding(positions)
        
        // Encoder layers
        for layer in encoderLayers {
            hidden = layer(hidden, mask: mask)
        }
        
        return encoderNorm(hidden)
    }
    
    func decode(_ inputIds: MLXArray, encoderOutput: MLXArray, selfMask: MLXArray? = nil, crossMask: MLXArray? = nil) -> MLXArray {
        let seqLen = inputIds.shape[1]
        let positions = MLXArray(0..<seqLen).reshaped([1, seqLen])
        
        // Embeddings
        var hidden = sharedEmbedding(inputIds)
        hidden = hidden + decoderPositionalEmbedding(positions)
        
        // Decoder layers
        for layer in decoderLayers {
            hidden = layer(hidden, encoderOutput: encoderOutput, selfMask: selfMask, crossMask: crossMask)
        }
        
        hidden = decoderNorm(hidden)
        return lmHead(hidden)
    }
    
    func generate(inputIds: MLXArray, maxLength: Int = 50, temperature: Float = 1.0) -> MLXArray {
        // Encode input
        let encoderOutput = encode(inputIds)
        
        // Start with BOS token
        var decoderInput = MLXArray([config.bosTokenId]).reshaped([1, 1])
        var generatedTokens: [Int32] = []
        
        for _ in 0..<maxLength {
            // Decode next token
            let logits = decode(decoderInput, encoderOutput: encoderOutput)
            let nextTokenLogits = logits[0, -1] // logits[0, -1, MLXArray(...)]
            
            // Apply temperature
            let scaledLogits = nextTokenLogits / temperature
            let probs = softmax(scaledLogits)
            
            // Sample next token
            let nextToken = max(probs).item(Int32.self)
            
            // Stop if EOS token
            if nextToken == config.eosTokenId {
                break
            }
            
            generatedTokens.append(nextToken)
            
            // Append to decoder input
            let newToken = MLXArray([nextToken]).reshaped([1, 1])
            decoderInput = concatenated([decoderInput, newToken], axis: 1)
        }
        
        return MLXArray(generatedTokens)
    }
}
